...
// 图表选项，用computed包裹，方便监听变量，每次有变量动态变化时，重新计算options的值
const options = computed(() => {
  const categories = dataList.value.map((item) => item.category)
  const countData = dataList.value.map((item) => item.count)
  const sizeData = dataList.value.map((item) => (item.totalSize / (1024 * 1024)).toFixed(2)) // 转为 MB，不然y轴范围非常大

  return {
    tooltip: { trigger: 'axis' },
    legend: { data: ['图片数量', '图片总大小'], top: 'bottom' },
    xAxis: { type: 'category', data: categories },// 横坐标
    yAxis: [
      {
        type: 'value',
        name: '图片数量',
